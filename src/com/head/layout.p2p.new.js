...
$(function () {
    ...
    $.getJSON(LufaxSite.Envs.evn.userStatic + "/service/user/current-user-info-for-homepage?jsoncallback=?",
        function (data) {
            var cookieName = lufax.Cookie.get("_tn");
            if (!data.userName) {
                if (!cookieName) {
                ...
                $('#my-account').find('a').after('<span class="msg-wrap">( <a href="https://my.' + location.hostname.split(".").slice(location.hostname.split(".").length-2).join(".") + '/my/user-msg" target="_blank"><i class="icon-msg"></i>' + unreadMsgCount + '</a> )</span>');
    ...
    if (!$(".foot-wrap").hasClass('foot-wrap-velocity')) {
