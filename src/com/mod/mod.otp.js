        ...
        return this.each(function () {
            ...
            var countDown = function() {
                ...
                if (obj.dynamicType == '2') {
                    ...
                    $otpInfo.html('您的手机' + obj.otpGetMobileNo() + '将在' + obj.otpVoiceWaitTime + '内收到<br/><span>' + obj.otpOfficialCall + '</span> 的电话，请接听并在120秒内输入');
                } else if (obj.dynamicType == '1') {
                    if(obj.otpMobileType == '1') {
                        $otpInfo.html('动态码已发送至您的手机' + obj.otpGetMobileNo() + '，请在120秒内输入，收不到短信请 <a href="javascript:void(0);" id="voiceObtainBtn">语音获取</a>');
                        $("body").off("click", '#voiceObtainBtn');
                        $("body").on("click", "#voiceObtainBtn", triggerVoiceObtainBtn);
                    } else {
                        $otpInfo.html('动态码已发送至您的手机' + obj.otpGetMobileNo() + '，请在120秒内输入');
                    }
                }
                $otpInfo.show();
            ...
            var handleSendVoiceOtpResponse = function(data) {
                if (data.retCode === "000") {
                    countDown();
                    $otpInfo.html('您的手机' + obj.otpGetMobileNo() + '将在' + obj.otpVoiceWaitTime + '内收到<span>' + obj.otpOfficialCall + '</span>的电话，请接听并在120秒内输入');
                    voiceInfoFlag = true;
                    setTimeout(function () {
                        voiceInfoFlag = true;
                        $otpInfo.hide();
